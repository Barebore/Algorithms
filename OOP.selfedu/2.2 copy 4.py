class RadiusVector2D:
    ...
    def __init__(self, x = 0 , y = 0):
        if self.MIN_COORD <= x <= self.MAX_COORD and (type(x) == float or type(x) == int):
            self.__x = x
        else:
            self.__x = 0
        if self.MIN_COORD <= y <= self.MAX_COORD and (type(y) == float or type(y) == int):
            self.__y = y
        else:
            self.__y = 0
    ...
    @x.setter
    def x(self, x):
        if (type(x) == float or type(x) == int) and self.MIN_COORD <= x <= self.MAX_COORD:
            self.__x = x
